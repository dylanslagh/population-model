...
**The Phase 4 Bayesian foundation is now in place.** The repository pins the
exact University of Washington annual fertility and life-expectancy archives,
checks the publisher's byte lengths, and records a SHA-256 fingerprint on the
first download. These are UW products aligned to WPP 2024, not official UN
...
This is infrastructure, not a Bayesian forecast yet. The native R archives
still need to be exported through their official accessors, the 236 UW
locations must be reconciled exactly against the project's 237 (the expected
omission is the Holy See, to be verified), and the conversion from TFR/e0 to
age schedules must be implemented and tested before any posterior population
range is published.
...
The next Phase 4 checkpoint is a one-country extraction fixture made through
the official `bayesTFR` and `bayesLife` accessors. That fixture must prove that
the exported values include UW's stored WPP alignment shifts, preserve the
original trajectory IDs, deliberately handle the 2023 anchor, and identify the
one location absent from the 236-location archive. After that comes the
versioned conversion from TFR/e0 to age-specific fertility and survival
schedules, followed by prior-predictive runs. The mechanistic layer where
selection competes against a falling fertility environment remains phase 5.
