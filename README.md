...
python -m pytest tests/ -q           # 29 tests, under a second, no data needed
```
...
The spec's build order asks for the backtest harness first — feed archived WPP
revisions in, get a score for how wrong each vintage was. That is skipped for
now because revisions before the mid-1990s exist only as scanned volumes, so
it's a research errand rather than a coding one. The engine was built first
because it is self-contained and testable today.

Open, in the spec's order: the map and population pyramids (phase 3), the
Bayesian layer starting from the University of Washington's published MCMC
objects (phase 4), and the mechanistic layer where selection competes against a
falling fertility environment (phase 5). `src/popmodel/scenarios.py` already
declares those scenarios with the phase that owes them, so the gap is visible in
