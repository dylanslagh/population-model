...
cohort-component model to 2150, working paper v1.2.2 and supplement, a concise
...
Phases 1 to 5 of the spec's six are done; phase 6 is scoring and cannot resolve
for years. Every number below is regenerated by a script; none is typed by hand.
...
**Which branch to work on, and when that changes.** Dylan's global rule sends
public projects through a branch and a preview before merge. He overrode it for
this repository on 2026-08-19, the day it went public: *"No need to preview it,
since there is no live site yet. It's in the hub and that's where it belongs for
now."* The rule exists so he can approve before the world sees a page, and no
world-visible page exists until the Cloudflare Pages project is created. So:
commit and push straight to `main`, and **revisit this the day that project is
created, and not before.** After that, branch and let him approve the preview.
...
committed. All eight sourced mechanism rows are verified. Do not reopen this by
fitting a latent-trait variance to the projection it is meant to explain.
...
**The remaining step is the unauthenticated launch itself**, which is a
Cloudflare Pages fact rather than a repository one: build command
`python3 scripts/build_public.py`, output directory `dist`, domain
`population.dylanslagh.com`. Until that is confirmed serving, do not write
anywhere that the site is live. The hub copy stays the review surface either
way.
...
`scripts/render_race.py` renders 1920x1080 frames and, with `--encode`, writes
`out/race-1950-2100.mp4`. It is published on YouTube and embedded on the front
page.
...
**Version 1.2.2, 2026-08-22.** *Selection on Fertility, and the Environmental
Decline That Would Cancel It.* Authored by Dylan Slagh, Claude Opus 5 and
ChatGPT 5.6 Sol, with a contributions statement that says what each of them did
and why two of them are on the byline at all. Two documents build: the paper and
a supplement carrying the engine validation, the parameter table, the backtest
and the reproduction path. Numbered PDFs of superseded versions stay in `paper/`
beside the current stable filenames.

**The paper is a standalone document, and that is a rule rather than a style.**
It does not narrate the project's own history, name the mistakes made on the way,
or refer to what an earlier version said. A reader who wants that has
`conversations/`, which is complete and public and exists precisely so the paper
does not have to carry it. If you find yourself adding "originally", "we first
tried", or "an earlier version of this model", the sentence belongs in the
conversation record instead.

### The three things it claims, in order of how much they rest on evidence

1. Measured mainstream selection is worth **1.82 billion** at 2150 (8.54 →
   10.36), from two independently sourced parameters. It shifts the level of
   the curve and does not restore growth.
...
The order of the argument is Dylan's ranking: the selection model leads, the
boundary is how selection is made quantitative, and the UN project extension is
the section that draws the 2100 line.

### Positions the paper holds that a future session might undo by accident

Each of these was argued for and is load-bearing. Changing one is allowed;
changing one without noticing is not.

* **Fertility and mortality are not frozen after 2100.** `src/popmodel/rates.py`
  continues both from the source's own 2070-2100 behaviour; see
  `docs/migration-extension.md`. Because the held-rate variant also exists, the
  cost of freezing is measured directly rather than argued about.
* **There are two break-even thresholds, not one.** Cancelling the terminal
  ...
  2.45%, because the extra people selection produced are still alive. Reporting
  only the first understates what it takes to undo the mechanism.
* **The named-group conclusion is narrow on purpose.** It supports a claim about
  the two modelled groups, not about high-fertility minorities in general.
* **The covariance is constructed, not observed.** The dispersion and the
  correlation come from different samples. The paper says so and names it as the
  weakest link in the calibration. Do not upgrade it to a heritability.
* **The parameter ensemble is a sensitivity envelope, not a posterior.** Uniform
  draws from judgemental ranges do not make a 90% credible interval, and the
  paper must not print one as though they did.
* **Transmission mode is a limitation, not a rival specification.** Anchoring
  transmission to a fixed 2024 baseline is not defensible over 126 years, so it
  is reported once as a lower bound.
* **The two full-selection curves nearly coincide; they do not coincide
  exactly.** The environment cancels out of the relative birth weights exactly,
  and out of the outcome very nearly: the world selection multipliers are 1.176
  and 1.174, a gap of 0.22%, because a harsher environment changes the age
  structure over which propensity is averaged and changes how forcefully a fixed
  migrant count mixes in. The paper, the figure and its footnote all say
  "almost". If you see "exactly" anywhere, it is wrong.
...
- `scripts/plot_paper_figures.py` writes vector PDFs into `paper/figures/` and a
  PNG of each into `out/paper-figures/`. **Open the PNGs.** Doing so catches
  what compiles and still looks wrong: bar lengths drawn on a log axis, a label
  sitting on top of a curve, a literal times-sign escape leaking into a figure.

**Build it with the repository's own script.** `scripts/build_paper.py` needs
Tectonic; it takes `$TECTONIC` or finds `tectonic` on `PATH`, and the local path
is in `LOCAL_TOOLS.md`. Versions 1.2.0 and 1.2.1 were compiled with pdfTeX in a
cloud sandbox instead, so their PDFs line-break differently from anything this
repository produces and are about twice the file size. Nothing in them is wrong
— same source, same macros, same figures — but do not treat a reflow against
those two files as a regression. From 1.2.2 the committed PDF is what the
committed script produces, which is the state to keep.
...
*Status updated 2026-08-22 on `main`: paper v1.2.2 and supplement are built; all
eight sourced mechanism parameters are verified; paired stochastic migration and
the selection-boundary robustness receipt passed; the YouTube video is published
and embedded; and the public site, the map and the paper landing page all build
and stage cleanly. The conversation record now covers all fifteen conversations
through 22 August and is regenerated by `scripts/export_conversations.py` rather
than edited by hand. What remains before launch is the Cloudflare Pages
deployment of `population.dylanslagh.com`; phase 6 scoring waits on WPP 2027 and
on completed-cohort fertility around 2038.*
